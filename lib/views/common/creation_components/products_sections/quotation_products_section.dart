...
class QuotationProductVariantsSection extends StatefulWidget {
  final String? branchId;
  final DefaultPrice? initPriceType;
  final List<OrderQuotationProducts> productVariants;
  final Function(List<ProductVariant>) onAddProductVariants;
  final Function(OrderQuotationProducts) onRemoveProductVariant;
  final Function(OrderQuotationProducts) onUpdateProductVariant;
  final Function(DefaultPrice)? onChangePriceType;
  final bool manageByQuantity;
  final List<String> quantityOpts;
  const QuotationProductVariantsSection({
    super.key,
    ...
    this.quantityOpts = const [],
  });

  @override
  State<QuotationProductVariantsSection> createState() =>
      _ProductVariantsSectionState();
}

class _ProductVariantsSectionState
    extends State<QuotationProductVariantsSection> {
  ...
  @override
  void didUpdateWidget(covariant QuotationProductVariantsSection oldWidget) {
    ...
    return Container(
      ...
      child: ColumnWidget(
        ...
        children: [
                ...
                return Column(
                  children: [
                    ...
                    if (productVariant != null)
                      QuotationVariantItem(
                        data: item,
                        manageByQuantity: widget.manageByQuantity,
                        quantityPrices: List.generate(
                          widget.quantityOpts.length,
                          (idx) => (
                            widget.quantityOpts[idx],
                            item.priceList != null &&
                                    (item.priceList?.length ?? 0) > idx
                                ? item.priceList![idx]
                                : productVariant.price,
                          ),
                        ),
                        onUpdateByOpts: widget.onUpdateProductVariant,
                        onRemove: () {
                        ...
                        onUpdateQuantity: (newQuantity) {
                          ...
                          widget.onUpdateProductVariant(item);
                        },
                      ),
                  ],
                );
          ...
          ExpandableWidget(
            initiallyExpanded: !widget.manageByQuantity,
            gap: 12.h,
            expandedChildren: [
              SizedBox.shrink(),
