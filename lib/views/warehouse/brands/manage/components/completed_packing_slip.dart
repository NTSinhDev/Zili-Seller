...
class _CompletedPackingSlipView extends StatelessWidget {
  final int index;
  const _CompletedPackingSlipView(this.index);

  static const PackingSlipStatus status = PackingSlipStatus.completed;
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                if (index == items.length) {
                  if (items.length <
                      warehouseRepository.totalCompletedPackings) {
                    return Padding(
                      padding: .all(20.w),
                      child: Center(
