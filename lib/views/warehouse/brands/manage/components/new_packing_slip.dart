...
class _NewPackingSlipView extends StatelessWidget {
  final int index;
  const _NewPackingSlipView(this.index);

  static const PackingSlipStatus status = PackingSlipStatus.newRequest;
  ...
  Widget build(BuildContext context) {
    ...
    return Offstage(
      ...
      child: StreamBuilder<List<PackingSlip>>(
        stream: warehouseRepository.newPackingsStream,
        initialData: warehouseRepository.newPackings,
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                if (index == items.length) {
                  if (items.length < warehouseRepository.totalNewPackings) {
                    return Padding(
                      padding: .all(20.w),
                      child: Center(
