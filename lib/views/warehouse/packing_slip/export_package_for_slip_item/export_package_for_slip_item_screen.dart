...
import '../../../product/select_package_variants/select_package_variants_screen.dart';
...
part 'components/item_mix_card.dart';
part 'components/item_mix_list_view.dart';

class ExportPackageForSlipItemScreen extends StatefulWidget {
  static const routeName = '/export-package-for-slip-item';
  final PackingSlipDetailItem slipItem;
  const ExportPackageForSlipItemScreen({super.key, required this.slipItem});

  @override
  State<ExportPackageForSlipItemScreen> createState() =>
      _ExportPackageForSlipItemScreenState();
}

class _ExportPackageForSlipItemScreenState
    extends State<ExportPackageForSlipItemScreen> {
  // ============================================================================
  // DEPENDENCIES
  // ============================================================================
  ...
  void dispose() {
    ...
    _prodRepository.packageVariants.sink.add([]);
            ...
            children: [
              ...
              ColumnWidget(
                ...
                children: [
                  ...
                  _ItemMixListView(widget.slipItem.itemMixes),
                ],
              ),
              // ================================================================
              ...
              StreamBuilder<List<ProductVariant>>(
                stream: _prodRepository.packageVariants.stream,
  ...
  void _exportWarehouseListener(BuildContext context, PackingSlipState state) {
    ...
    if (state is PackingSlipSuccess &&
        state.action == PackingSlipAction.exportPackage) {
  ...
  Future<void> _onSubmit(BuildContext context) async {
    ...
    // ========================================================================
    final variants = _prodRepository.packageVariants.valueOrNull;
    ...
    for (final variant in variants) {
      ...
      if (fieldKey == null) {
        ...
        return _showError(
          context,
          'Vui lòng nhập khối lượng cho bao bì "${renderProductVariantName(variant, variant.options)}"',
        );
      }
    ...
    if (totalWeight <= 0) {
      return _showError(context, 'Tổng số lượng phải lớn hơn 0');
    }
    ...
    await showNoticeDialog(
      context: context,
      title: 'Xuất kho bao bì',
      message: 'Xác nhận xuất kho bao bì từ các thông tin đã chọn?',
      ...
      primaryAction: NoticeDialogAction(
        ...
        onPressed: () async {
          ...
          await _cubit.exportWarehousePackage(
  ...
  void _handleCancel(BuildContext context) {
    ...
    if (_prodRepository.packageVariants.valueOrNull?.isNotEmpty == true) {
      // Có dữ liệu -> Hiển thị dialog xác nhận
      showNoticeDialog(
        context: context,
        title: 'Thoát khỏi trang?',
        message:
            'Thông tin xuất kho bao bì sẽ mất, xác nhận để thoát khỏi trang.',
        variant: NoticeVariant.confirm,
        secondaryAction: NoticeDialogAction(
          label: 'Hủy',
          onPressed: context.navigator.pop,
